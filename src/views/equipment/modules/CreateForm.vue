  ...
  >
    <div class="drawer-wrap">
        ...
        <a-spin :spinning="loading">
          <a-form :form="form" v-bind="formLayout">
            ...
            <a-form-item label="器材标题">
              <a-input
                v-decorator="['title', {rules: [{required: true, min: 2, max: 12, message: '请输入至少2字符，最多14个字符'}]}]"
                placeholder="请输入器材标题" />
            </a-form-item>
            <a-form-item label="器材封面">
              ...
              <a-upload
                v-decorator="['cover', {rules: [{required: true, message: '请上传器材封面'}]}]"
                accept="image/*"
            ...
            <a-form-item label="器材类型">
              <a-select
                v-decorator="['equipment_type', {rules: [{required: true, trigger: 'change', message: '请选择器材类型'}]}]"
                placeholder="请选择">
            ...
            <a-form-item label="是否轮播">
              <a-select
                v-decorator="['carousel', {rules: [{required: true, trigger: 'change', message: '请选择是否轮播'}]}]"
                placeholder="请选择">
            ...
            <a-form-item label="器材长图">
              ...
              <a-upload
                ...
                v-decorator="['long_figure', {rules: [{required: true, message: '请上传器材长图'}]}]"
                accept="image/*"
                :file-list="fileListLong"
                :remove="handleRemoveLong"
                :before-upload="beforeUploadLong">
      ...
      <div class="drawer-footer">
        ...
        <a-button type="primary" :loading="loading || checkLoading" @click="() => { $emit('ok') }">{{ checkLoading ? '校验中' : '确定' }}</a-button>
      </div>
    </div>
